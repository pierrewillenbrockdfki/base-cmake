...
macro (rock_find_pkgconfig VARIABLE)
    ...
    if (NOT ROCK_FEATURE_NOCURDIR)
        ...
        message(STATUS "Adding definitions and include directories for ${VARIABLE}")
    endif()
endmacro()
...
macro(rock_target_setup TARGET_NAME)
    ...
    foreach (plain_dep ${${TARGET_NAME}_DEPS_PLAIN})
        target_link_libraries(${TARGET_NAME} ${${plain_dep}_LIBRARIES}
            ${${plain_dep}_LIBRARY})
        message(STATUS "Plain library(s) for ${TARGET_NAME}: ${plain_dep}: ${${plain_dep}_LIBRARIES} ${${plain_dep}_LIBRARY}")
    endforeach()
    if (ROCK_FEATURE_NOCURDIR)
        foreach (plain_dep ${${TARGET_NAME}_DEPS_PLAIN} ${${TARGET_NAME}_PUBLIC_PLAIN})
            ...
            message(STATUS "Plain definition(s) for ${TARGET_NAME}: ${plain_dep}: ${_filtered_defs}")
            message(STATUS "Plain options(s) for ${TARGET_NAME}: ${plain_dep}: ${_filtered_opts}")
            message(STATUS "Plain feature(s) for ${TARGET_NAME}: ${plain_dep}: ${_filtered_feats}")
            message(STATUS "Plain include(s) for ${TARGET_NAME}: ${plain_dep}: ${${plain_dep}_INCLUDE_DIRS}")
        endforeach()
    endif()
    foreach (pkgconfig_pkg ${${TARGET_NAME}_DEPS_PKGCONFIG})
        target_link_libraries(${TARGET_NAME} ${${pkgconfig_pkg}_PKGCONFIG_LIBRARIES})
        message(STATUS "Pkgconfig library(s) for ${TARGET_NAME}: ${pkgconfig_pkg}: ${${pkgconfig_pkg}_PKGCONFIG_LIBRARIES}")
    endforeach()
    if (ROCK_FEATURE_NOCURDIR)
        foreach (pkgconfig_pkg ${${TARGET_NAME}_DEPS_PKGCONFIG} ${${TARGET_NAME}_PUBLIC_PKGCONFIG})
            ...
            message(STATUS "Pkgconfig definition(s) for ${TARGET_NAME}: ${pkgconfig_pkg}: ${_filtered_defs}")
            message(STATUS "Pkgconfig option(s) for ${TARGET_NAME}: ${pkgconfig_pkg}: ${_filtered_opts}")
            message(STATUS "Pkgconfig feature(s) for ${TARGET_NAME}: ${pkgconfig_pkg}: ${_filtered_feats}")
            message(STATUS "Pkgconfig include(s) for ${TARGET_NAME}: ${pkgconfig_pkg}: ${${pkgconfig_pkg}_PKGCONFIG_INCLUDE_DIRS}")
        endforeach()
    endif()
    foreach (__cmake_target ${${TARGET_NAME}_DEPS_TARGET})
        target_link_libraries(${TARGET_NAME} ${__cmake_target})
        message(STATUS "Cmake library(s) for ${TARGET_NAME}: ${__cmake_target}")
    endforeach()
    foreach (imported_dep ${${TARGET_NAME}_IMPORTED_DEPS})
        target_link_libraries(${TARGET_NAME} ${${imported_dep}_LIBRARIES})
        message(STATUS "Imported library(s) for ${TARGET_NAME}: ${imported_dep}: ${${imported_dep}_LIBRARIES}")
    endforeach()
    foreach (internal_dep ${${TARGET_NAME}_DEPS})
    ...
    message(STATUS "DEPS library(s) for ${TARGET_NAME}: ${${TARGET_NAME}_DEPS}")
    target_link_libraries(${TARGET_NAME} ${${TARGET_NAME}_DEPENDENT_LIBS})
    message(STATUS "LIBS library(s) for ${TARGET_NAME}: ${${TARGET_NAME}_DEPENDENT_LIBS}")
    foreach (cmake_pkg ${${TARGET_NAME}_DEPS_CMAKE})
        ...
        message(STATUS "CMAKE library(s) for ${TARGET_NAME}: ${cmake_pkg}: ${${cmake_pkg}_LIBRARIES} ${${cmake_pkg}_LIBRARY}")
        target_link_libraries(${TARGET_NAME} ${${UPPER_cmake_pkg}_LIBRARIES} ${${UPPER_cmake_pkg}_LIBRARY})
        message(STATUS "CMAKE library(s) for ${TARGET_NAME}: ${UPPER_cmake_pkg}: ${${UPPER_cmake_pkg}_LIBRARIES} ${${UPPER_cmake_pkg}_LIBRARY}")
    endforeach()
    if (ROCK_FEATURE_NOCURDIR)
        foreach (cmake_pkg ${${TARGET_NAME}_DEPS_CMAKE} ${${TARGET_NAME}_PUBLIC_CMAKE})
            ...
            rock_find_cmake(${cmake_pkg} REQUIRED)
            message(STATUS "CMAKE definition(s) for ${TARGET_NAME}: ${cmake_pkg}: ${_filtered_defs}")
            message(STATUS "CMAKE options(s) for ${TARGET_NAME}: ${cmake_pkg}: ${_filtered_opts}")
            message(STATUS "CMAKE feature(s) for ${TARGET_NAME}: ${cmake_pkg}: ${_filtered_feats}")
            message(STATUS "CMAKE include(s) for ${TARGET_NAME}: ${cmake_pkg}: ${${cmake_pkg}_INCLUDE_DIRS}")
        endforeach()
        list(LENGTH ${TARGET_NAME}_UI QT_UI_LENGTH)
        if (QT_UI_LENGTH GREATER 0)
