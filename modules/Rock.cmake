...
macro(rock_target_definition TARGET_NAME)
    ...
    if (QT_MOC_LENGTH GREATER 0)
        ...
        set(CMAKE_CURRENT_BINARY_DIR "${CMAKE_CURRENT_BINARY_DIR}-qt4")
    ...
    if (QT_MOC5_LENGTH GREATER 0)
        ...
        set(__save_bin_dir ${CMAKE_CURRENT_BINARY_DIR})
        set(CMAKE_CURRENT_BINARY_DIR "${CMAKE_CURRENT_BINARY_DIR}-qt5")
    ...
    if (QT_UI5_LENGTH GREATER 0)
        ...
        if (NOT ROCK_FEATURE_NOCURDIR)
            include_directories(${CMAKE_CURRENT_BINARY_DIR}-qt5)
        endif()
...
macro(rock_filter_cflag_definitions DEFS_VAR OPTS_VAR)
...
macro(rock_target_setup TARGET_NAME)
    ...
    if (ROCK_FEATURE_NOCURDIR)
        foreach (plain_dep ${${TARGET_NAME}_DEPS_PLAIN} ${${TARGET_NAME}_PUBLIC_PLAIN})
            rock_filter_cflag_definitions(_filtered_defs _filtered_opts ${${plain_dep}_CFLAGS_OTHER})
    ...
    if (ROCK_FEATURE_NOCURDIR)
        foreach (pkgconfig_pkg ${${TARGET_NAME}_DEPS_PKGCONFIG} ${${TARGET_NAME}_PUBLIC_PKGCONFIG})
            rock_filter_cflag_definitions(_filtered_defs _filtered_opts ${${pkgconfig_pkg}_PKGCONFIG_CFLAGS_OTHER})
            target_compile_definitions(${TARGET_NAME} PUBLIC ${_filtered_defs})
            target_compile_options(${TARGET_NAME} PUBLIC ${_filtered_opts})
    ...
    if (ROCK_FEATURE_NOCURDIR)
        foreach (cmake_pkg ${${TARGET_NAME}_DEPS_CMAKE} ${${TARGET_NAME}_PUBLIC_CMAKE})
            rock_filter_cflag_definitions(_filtered_defs _filtered_opts ${${cmake_pkg}_CFLAGS_OTHER})
            target_compile_definitions(${TARGET_NAME} PUBLIC ${_filtered_defs})
            target_compile_options(${TARGET_NAME} PUBLIC ${_filtered_opts})
        ...
        list(LENGTH ${TARGET_NAME}_UI QT_UI_LENGTH)
        if (QT_UI_LENGTH GREATER 0)
            target_include_directories(${TARGET_NAME} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}-qt4)
        endif()
        list(LENGTH ${TARGET_NAME}_UI5 QT_UI5_LENGTH)
        if (QT_UI5_LENGTH GREATER 0)
            target_include_directories(${TARGET_NAME} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}-qt5)
        endif()
    endif()
endmacro()
...
function(rock_vizkit_plugin TARGET_NAME)
    if (${PROJECT_NAME} STREQUAL "vizkit3d")
        ...
        if ((DEFINED ROCK_QT_VERSION_5) AND (NOT DEFINED ROCK_QT_VERSION_4))
            message(SEND_ERROR "You are using the qt4 rock_vizkit_plugin instead of rock_vizkit_plugin_qt5 while only having looked for qt using rock_find_qt5. Use rock_vizkit_plugin_qt5, suffix the library with an additional -qt5 and adjust the .pc file accordingly. Better yet, add rock_find_qt4(NOCURDIR), and duplicate your rock_vizkit_plugin call for qt5.")
